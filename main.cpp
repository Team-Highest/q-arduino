...
void run_vision_inference(cv::Mat frame) {
    std::cout << "[Vision] Running inference on frame: " 
              << frame.cols << "x" << frame.rows << " px" << std::endl;
}
        ...
        .message = [](auto *ws, std::string_view message, uWS::OpCode opCode) {
            ...
            if (opCode == uWS::OpCode::BINARY && !message.empty()) {
                ...
                if (header == 0x01) {
                    ...
                    if (!frame.empty()) {
                        run_vision_inference(frame);
                    } else {
                        std::cerr << "Failed to decode JPEG payload." << std::endl;
                    }
                } 
                else if (header == 0x02) {
                    // --- AUDIO PAYLOAD ---
